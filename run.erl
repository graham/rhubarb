...
main(_) ->
    ...
    wait(3000),
    run_and_time("read              ", fun(I) -> key:read(I) end, ListOfProcs),
    run_and_time("query             ", fun(I) -> key:key_query(I, max) end, ListOfProcs),
    ...
    run_and_time("write->rpush      ", fun(Count) -> bucket:write(Bucket, "woot" ++ [Count], rpush, 100) end, ListOfProcs2),
    run_and_time("write->rpop       ", fun(Count) -> bucket:write(Bucket, "woot" ++ [Count], rpop, nil) end, ListOfProcs2),
    run_and_time("write->lpush      ", fun(Count) -> bucket:write(Bucket, "woot" ++ [Count], lpush, 100) end, ListOfProcs2),
    run_and_time("write->lpop       ", fun(Count) -> bucket:write(Bucket, "woot" ++ [Count], lpop, nil) end, ListOfProcs2),
    run_and_time("blind_write->rpush", fun(Count) -> bucket:write(Bucket, "woot" ++ [Count], rpush, 200) end, ListOfProcs2),
    run_and_time("read              ", fun(Count) -> bucket:read(Bucket, "woot" ++ [Count]) end, ListOfProcs2),
    wait(3000),
    run_and_time("read              ", fun(Count) -> bucket:read(Bucket, "woot" ++ [Count]) end, ListOfProcs2),
    %run_and_time("query             ", fun(Count) -> bucket:search(Bucket, "woot" ++ [Count], max) end, ListOfProcs2),
    ok.
